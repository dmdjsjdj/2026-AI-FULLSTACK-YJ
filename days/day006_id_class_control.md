...
2. 제목 글자를 초록색으로 바꾸려면?   h1{color: green;}
3. 본문 글자의 크기를 18px로 지정하려면?  div{font-size: 18px;}
4. 메뉴 글자를 오른쪽으로 정렬하려면?   p{text-align: center;}
5. 링크에 밑줄을 없애려면?    a{text-decoration: none;}
6. 글꼴을 'Times New Roman'으로 바꾸려면?   font-family: 'Times New Roman';
7. 강조하고 싶은 단어를 굵게 표시하려면?   font-weight: bold;
8. 이미지를 가로 400px로 줄이고 싶다. 어떤 속성?   img{width: 400px;}
9. 박스 바깥쪽에 20px 여백을 주려면?   margin: 20px;
10. 버튼 안쪽에 8px 여백을 주려면?    button{padding: 8px;}
11. 카드 요소에 2px 점선 테두리를 주려면?  .card{border: 2px solid blue;}
12. 프로필 사진 모서리를 둥글게 50%로 만들려면?   img{border-radius:50%;}
13. 박스에 살짝 번지는 그림자 효과를 주려면?   box-shadow: 0 0 12px rgba(0,0,0,0.3);
