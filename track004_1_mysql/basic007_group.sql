...
insert into group_userinfo value (1,'first', 11, null, 100, 100, 99, 'A', 'n');
insert into group_userinfo value (2,'second', 22, 'm', 89, 92, 78, 'B', 'y');
insert into group_userinfo value (3,'third', 33, 'm', 90, 92, 97, 'A', 'y');
insert into group_userinfo value (4,'fourth', 44, 'f', 40, 42, 67, 'C', 'n');
insert into group_userinfo value (5,'fifth', 55, 'f', 89, 86, 99, 'B', 'y');
insert into group_userinfo value (6,'sixth', 66, 'm', 10, 20, 44, 'C', 'n');
...
from group_userinfo; 
-- 10:42:44	select ban,count(*)  from group_userinfo LIMIT 0, 1000	Error Code: 1140. In aggregated query without GROUP BY, expression #1 of SELECT list contains nonaggregated column 'mbasic.group_userinfo.ban'; this is incompatible with sql_mode=only_full_group_by	0.000 sec
...
-- +------+----------+----------+----------+
-- 3 rows in set (0.00 sec)
select 		ban, avg(kor) `국어총합`, avg(eng) `영어총합`, avg(math) `수학총합`
from 		group_userinfo
group by 	ban;
--  
...
select	max(sal) `부서10 최대급여`  from 	emp  group by deptno having	deptno=10;
--  
...
--   FROM EMP
--   WHERE AVG(SAL) >= 2000  	전체 행을 읽으면서 실행되는 주건문 / 집계함수 사용불가
--   GROUP BY DEPTNO, JOB
--   ORDER BY DEPTNO, JOB;
select	deptno,job, avg(sal) `평균급여`
from 	emp
