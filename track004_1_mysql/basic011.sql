...
where d.deptno=20 and sal>(select avg(sal) from emp)
order by empno ;
-- +-------+-------+---------+------+--------+----------+--------+
...
-- select    *
-- from      emp
-- where     sal   (select max(sal) in  (  각 부서별 최고 급여  )
...
select   num 
from     atest  
where    num > all(    select num  from atest  where num in(3,4,5)    )  -- all 모두 5보다 작다
order by num;
...
select  * 
from    emp e1 
where   sal < any(  select  max(sal)  from   emp  e2    where  e2.deptno=30 group by deptno);
...
select * from emp where sal > any(select min(sal) from emp where deptno=30 group by deptno) order by sal desc;
-- +-------+--------+-----------+------+------------+------+------+--------+
-- | empno | ename  | job       | mgr  | hiredate   | sal  | comm | deptno |
-- +-------+--------+-----------+------+------------+------+------+--------+
-- |  7839 | KING   | PRESIDENT | NULL | 1981-11-17 | 5000 | NULL |     10 |
