...
-- 1. 시스템의 현재 시각 조회 
select now();		 -- '2026-05-26 14:38:17'
select current_time(); -- '14:37:58'
...
select date_format( '2026-05-26', "%Y-%m-%d" );
...
select timestampdiff( hour, "2026-05-26", "2026-05-25" );  -- 24시간
...
-- +------+---------------------+
select * from date_userinfo where date < '2022-11-01';
...
-- +------+---------------------+---------------------+
select name, date, (select date_add( date, interval  +30 day )) as `event` from date_userinfo 
...
-- >> 연습문제2)  select_emp
-- > 1. 금일날짜를 출력하시오.
select date_format(now(), "%m-%d") `금일날짜`;
...
-- > 4. 현재날짜를 기준으로 가장 가까운 오는 일요일의 날짜를 구하시오.  now(), weekday(), date_format, date_add, datediff 
...
create table control (no int null);
desc control;
...
-- if (조건, 참, 거짓)
-- 1이다, 1 아니다
 select no, if(no=1,'1이다', '1이 아니다') `1상태` from control;
...
create table if_userinfo select * from group_userinfo;
update if_userinfo set age=122 where no=2;
alter table if_userinfo modify no int not null primary key auto_increment;
alter table if_userinfo modify name varchar(100) not null;
alter table if_userinfo modify age int null default 0;
alter table if_userinfo modify sns char(1) null;
alter table if_userinfo modify sex char(1) null;

select * from if_userinfo;
desc if_userinfo;
ALTER TABLE if_userinfo
DROP COLUMN kor,
DROP COLUMN eng,
DROP COLUMN math,
DROP COLUMN ban;
...
-- +----+--------+-----+------+------+----------+
-- 6 rows in set (0.00 sec)
select *, if(age>19,'성인','미성년자') `성인여부` from if_userinfo;
...
-- |             3 |
-- +---------------+
select count(*) `sns수신거부수` from if_userinfo
where sns='n';
