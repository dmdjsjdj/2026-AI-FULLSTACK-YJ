...
insert into userinfo values ('aaa',    11, 1);  -- error
-- 10:35:51	insert into userinfo values ('aaa',    11, 1)	
...
-- 10:41:40	insert into userinfo (name, age) values (22, 'bbb')	
...
SET SQL_SAFE_UPDATES=0;  -- SAFE 안씀
...
update userinfo set name='fourth', age=44 where no=4 and name='first';  -- userinfo의 no=4의 이름은 'fourth'라 조건에 안맞아서 실행x
...
alter table score modify ssavg double;  -- ssavg double로 변경->소수점 출력
...
update score set semail='hello@gmail.com' where ssavg=89.5 and sproject=98; -- 조건 안맞음
...
alter table userinfo2 modify no int not null auto_increment primary key;  -- no auto_increment primary key 추가
...
-- mysql>
desc emp;
create table del_emp select * from emp;
alter table del_emp modify empno int not null  auto_increment primary key;
select * from del_emp;

-- [001] [TABLE : delete_emp] (직책(JOB)이  'SALESMAN'인 데이터를 삭제하시오.  ) 
delete from del_emp where job='SALESMAN';
-- [002] [TABLE : delete_emp] (직책(JOB)이  'MANAGER'이고  이름(ENAME)이 'JONES'인 데이터를 삭제하시오.  ) 
delete from del_emp where job='MANAGER' and ename='JONES';
