...
show tables;  -- 테이블목록확인
alter table select_userinfo modify no int not null primary key auto_increment; -- no primary key, auto_increment 추가
desc select_userinfo;  -- 테이블구조확인
select * from select_userinfo;

delete from select_userinfo where no>=5; -- 5 이상 테이터 삭제
...
-- SELECT   검색필드1, 검색필드2,,,
-- FROM    테이블명
-- WHERE where_condition -- 조건없으면 생략가능
-- ORDER BY  기준필드  [ASC | DESC] -- 기본이 asc라서 생략가능
-- LIMIT offset;
...
from select_userinfo 
order by age desc 
limit 0,2;  -- 어디서부터, 몇개   ( 첫번째부터, 2개)

select no, age
from select_userinfo 
order by age desc 
limit 1,2;  -- 어디서부터, 몇개   ( 두번째부터, 2개)

-- 실습 1.  emp테이블을  복사하시오. (있다면 pass)
...
-- 14 rows in set (0.00 sec)
--  * 컬럼명에 별칭주기(as) 
select empno as `사원번호`, ename as `이름`, job as `담당 업무` from emp;
...
-- +-------------+---------------+-----------+---------+------------+--------+------------+---------------+
-- 14 rows in set (0.00 sec)
select empno as `EMPLOYEE_NO`, ename as `EMPLOYEE_NAME`, job as `JOB`, mgr as `MANAGER`, hiredate as `HIREDATE`, sal as `SALARY`, comm as `COMMISSION`, deptno as `DEPARTMENT_NO` 
from emp
