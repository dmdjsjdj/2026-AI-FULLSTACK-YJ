 ...
 * - axior로 API를 호출 -> 성공/실패에 따라 reducer로 액션 전달
 */
...
// 3. call - api 호출하고 결과를 기다림 (blocking) > 동기 
...
function* watchLogin(){
    ...
    // LOG_IN_REQUEST 액션발생 -> 여러번 요청시 가장 마지막 요청처리 1개
    // return { ...state, isLoading: true, error: null }
}
// 1. all  - 여러 saga를 동시에 실행
// 2. fork - [비동기]로 saga 실행
...
function* watchSignup(){
    ...
    // SIGN_UP_REQUEST 액션발생 -> 여러번 요청시 가장 마지막 요청처리 1개
    // return { ...state, isLoading: true, error: null }
}
// 1. all  - 여러 saga를 동시에 실행
// 2. fork - [비동기]로 saga 실행
...
function* watchLoadUsers(){
    ...
    // LOAD_USERS_REQUEST 액션발생 -> 여러번 요청시 가장 마지막 요청처리 1개
    // return { ...state, isLoading: true, error: null }
}
// 1. all  - 여러 saga를 동시에 실행
// 2. fork - [비동기]로 saga 실행
...
export function* checkNickname(action) {
    try {
        const result = yield call(checkNicknameApi, action.data.nickname);
