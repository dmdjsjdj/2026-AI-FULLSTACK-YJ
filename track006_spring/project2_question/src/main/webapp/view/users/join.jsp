   		...
   		window.addEventListener("load", function() {
			...
			nickname.addEventListener("keyup", function(e) {  //console.log(e.target.value);
				...
				if(value != ""){  // 빈칸이 아니면 서버에 요청
					fetch("${pageContext.request.contextPath}/users/nicknameCheck?nickname="
					...
					tnickname.textContent="닉네임 입력은 필수입니다.";
					tnickname.className  ="my-3 alert alert-warning tnickname"
				} // 빈칸이면 메세지 유지
			});
     ...
     <script>
   		window.addEventListener("load", function() {
			...
			email.addEventListener("keyup", function(e) {  //console.log(e.target.value);
				let value = e.target.value.trim(); // 공백빼기
				if(value != ""){  // 빈칸이 아니면 서버에 요청
					fetch("${pageContext.request.contextPath}/users/emailCheck?email=" 
							+ encodeURIComponent(value))
					.then( response=> response.json()) // json으로 응답
					.then( data=>{ 
						if(data.exists){
