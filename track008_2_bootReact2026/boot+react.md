...
2) spring security - 인증, 인가/ 필터체인의 요청 보호 / oauth2.0
3) mybatis         - xml sql 쿼리 작성
4) jwt             - json web token  / 토큰기반의 인증방식
                     토근안에 사용자의 정보와 권한을 담아 전달,
                     서버가 세션을 직접 관리 하지않고, 
                     클라이언트가 코튼 보관
5) redis           - 캐시/세션 관리, refresh token을 저장,
                     캐싱처리(자주 사용하는 값을 미리 넣어놓고 요청이 있을때 
                     서버를 거치지 않고 빠르게 제공)에 활용
...
##### [실습]  5.  Boot + React + 세션/쿠키 - ver2 ( 기본 게시판 + 회원가입 + 이미지 + / 해시태그/ 좋아요 / 팔로우 )
※ entity -> repository -> service -> controller
##### [실습]  6.  Boot + React + jwt + security+redies - ver3 ( 기본 게시판 + 회원가입 + 이미지 + / 해시태그/ 좋아요 / 팔로우 )
...
<Link href="/signup">     join.js       # 회원가입 페이지
<Link href="/posts/new">  posts/new.js  # 글쓰기 페이지
