...
public interface PostLikeRepository extends JpaRepository<PostLike, Long>{
	...
	// 특정유저가 특정게시글에 좋아요했는지 AppUser user 필드와 Post post 각각의 id가 있는지 확인
	// 엔티티(AppUser)와 필드명(user)이 다를때
	long countByUser_IdAndPost_Id(Long userId, Long postId);
	
	// 특정유저가 특정게시글에 좋아요했는지 조회
	...
	//			-> select (데이터베이스 조회) delete(개별삭제)
	//방법2: @Query (select 조회용도)  -> db가서 바로 delete
