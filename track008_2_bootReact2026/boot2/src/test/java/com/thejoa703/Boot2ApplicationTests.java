...
class Boot2ApplicationTests {
    ...
    // insert : save / select:findBy / update:save / delete:delete
    @Test 
    @DisplayName("■ CommentRepositoty-CRUD")
    void testCommentRepositoty() {
    	// 댓글 생성
    	...
    	// 댓글 조회
    	List<Comment> comments = commentRepositoty.findByPostIdAndDeletedFalse(post.getId());
    	assertThat(comments.size()).isEqualTo(1);
    	
    	// 댓글 수정
    	comment.setContent("수정된 댓글");
    	commentRepositoty.save(comment);
    	assertThat(commentRepositoty.findById(comment.getId()).get().getContent())
    				.isEqualTo("수정된 댓글");
    	
    	// 댓글 삭제
    	comment.setDeleted(true);
    	commentRepositoty.save(comment);
    	List<Comment> rcomments = commentRepositoty.findByPostIdAndDeletedFalse(post.getId());
    ...
    void testPostLikeRepository() {
    	...
    	// 특정유저가 특정게시글에 좋아요했는지
    	Optional<PostLike> found = postLikeRepository
    			.findByUser_IdAndPost_Id(user2.getId(), post.getId());
