...
public class Chat001_Server {
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("[Server] 4. 고객님 연락와서 상담사(socket) 연결함.....");
...
class Receiver extends Thread {  
	...
	@Override public void run() { 
		try { while(in != null) { System.out.println( in.readUTF()); } } 
							...
							/* e.printStackTrace(); 종료할때 에러문구 안나오게 지움 */ }
		finally {
			try {
				if( 	in != null ) { in.close(); }
...
class Sender extends Thread { 
	...
	public Sender(Socket socket) { 
		this.socket = socket; // 상대방과 연결되어있는 정보
		try {
		...
		try {
			while(out != null) {
				...
				out.writeUTF(this.who + time +"   " + br.readLine() );
							...
							/* e.printStackTrace(); 종료할때 에러문구 안나오게 
							 * 지움 */ }
		finally {
			try {
...
2. Socket 통신 - 양방향 (특정 포트를 통해 실시간으로 정보 주고받음 - tcp/udp)
3. 소켓통신흐름
	1) 서버소켓( as 센터 ), 포트바인딩( 문열기 )
	2) 클라이언트 연결 요청, 수락
	3) 클라이언트 소켓(socket) ↔ 상담사(socket)가 읽어들이기 (InputStream  > 프로그램 기준 >  OutputStream)
