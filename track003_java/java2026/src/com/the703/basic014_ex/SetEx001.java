...
public class SetEx001 {
	...
	public static void main(String[] args) {
		...
		Set<UserInfo2> mavl = new HashSet<>();
		UserInfo2 info = new UserInfo2();
		String find = ""; double avg=0;
		
		mavl.add(new UserInfo2(1, "아이언맨" , 50));
		mavl.add(new UserInfo2(2, "헐크" , 40));
		mavl.add(new UserInfo2(3, "캡틴" , 120));
		mavl.add(new UserInfo2(3, "캡틴" , 120));
		
		for(UserInfo2 m : mavl) {
			System.out.println(m.getNo() + " - " + m.getName() + " - " + m.getAge());
			avg+=m.getAge();
		}
		System.out.print("> 찾을 유저이름 : ");
		find = sc.next();

		//만약 유저의 이름이 find와 같다면 유저줄력
//		System.out.println( mavl.contains(new UserInfo2(3, find , 120)) );
		
		for(UserInfo2 m : mavl) {
			//if(만약 유저이름이 find와 같다면 - equals)
			if(m.getName().equals(find)) {
				System.out.println(m);
				break;
			}
			
		}System.out.println("나이평균 > "+avg/mavl.size());
