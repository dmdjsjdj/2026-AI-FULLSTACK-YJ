...
//정의 - 외부에서 접근제어 
//구현 - balance private 선언 / deposit, withdraw, getBalance 메서드 통해서만 접근가능
...
//Q6. a1.id와 a2.id의 값은 각각 얼마인가? 1, 2
...
//변수- 값변경불가(상수) / 메서드-오버라이드불가 / 클래스-상속불가
...
//1. 클래스는 부품객체
//2. 상태(멤버변수:클래스변수, 인스턴스변수)와 행위(멤버변수:클래스메서드, 인스턴스메서드)
class Account {
...
Q1. 캡슐화(Encapsulation)란 무엇이며, 위 코드에서 어떻게 구현되었는지 설명하시오.
하나로 묶고 외부접근 제한 -  private 선언 / getBalance, deposit, withdraw
Q2. 접근제어자의 범위를 넓은 것부터 좁은 것까지 순서대로 쓰시오. public > protected > default(package > private
Q3. static 키워드의 의미를 메모리 구조와 연결지어 설명하고, 위 코드에서 어떤 변수에 적용되었는지 쓰시오.
가장먼저 올림  - accountCount
Q4. final 키워드의 의미를 설명하고, 위 코드에서 어떤 변수에 적용되었는지 쓰시오.
상속금지 - id
Q5. Account.accountCount의 값은 얼마인가? 왜 그렇게 되는지 설명하시오.
2 / public Account(){this.id = ++accountCount;} 아이디가 추가될떄마다 카운트-a1,a2 두개가  올라감
Q6. a1.id와 a2.id의 값은 각각 얼마인가? 1, 2
Q7. 출력 결과를 쓰시오.
150
170
...
a2.id = 2
Q8. static 메서드와 인스턴스 메서드의 차이를 설명하시오.
static 메서드는 가장 먼저 올라가는 값으로 this사용 불가
인스턴스 메서드는 this사용 가능
Q9. final 키워드가 변수, 메서드, 클래스에 각각 적용될 때 의미를 설명하시오.
변수- 값변경x
메서드-오버라이드x
