...
/*
Q3,Q4. 상속도를 그리시오. / 각 객체에서 사용할 수 있는 멤버변수와 메서드를 적으시오.
			Object
			  ↑
...
//	Q6. Parent 클래스의 멤버변수/메서드의 종류와 메모리 구조를 설명하시오.
    int x = 100;	// 인스턴스 변수 - heap area - new - 생성자 - this각각
...
//    	1. new-객체생성, 주소 / Child()-초기화 / p(주소값) x=100, method() 
    	
//    	method area (정보,static,final) - Parnet.class,   Child.class,  #1)Repeat001_oop.class
//    	  heap										stack
//    	  
...
//    	Parent() {x=100, method() -> Parent method}
//    	Child()	 {x=200, @method() -> Child method }
//    	#3)1번지)Parent{x=100, method()}			 <- p [1번지] #5)
//    	  											[main]#2)
        Parent p = new Child();
        Child c = new Child();
...
//        Q7. 출력 결과를 쓰시오. 
        System.out.println("p.x = " + p.x); // p.x의 값은? 100 
        p.method(); // p.method() 실행 시 어떤 메서드가 호출되는가? method(Child method) 

        System.out.println("c.x = " + c.x); // c.x의 값은? 200 
        c.method(); // c.method() 실행 시 어떤 메서드가 호출되는가? method(Child method)
    }
}
