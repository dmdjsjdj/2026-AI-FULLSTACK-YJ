...
public class Bank_v3_손예진 {
		...
		//		      - 삭제
		//		      - 비번변경
		public static void upmu(int f, int m) {
			if(f!=-1) {
				switch( m ){
	            case 2 : System.out.printf("ID : %s\nPASS: %s\nBALANCE: %.2f\n" ,id[f] ,pass[f] , balance[f]);     break;
	            case 3 : System.out.print("입금할 금액 > ");  tempbalance = sc.nextInt();       
	                     System.out.println("입금완료! 현재잔액 : " + (balance[f] += tempbalance));
	            
	            break;
	            case 4 :  
	               System.out.print("출금할 금액 > ");   tempbalance = sc.nextInt();  
	               System.out.println( tempbalance > balance[f] ?"잔액부족! 출금불가" : "출금완료! 현재잔액 : " + (balance[f] -= tempbalance));
	            break;
	            case 5 : System.out.print("계좌삭제 (Y/N) > "); char again = sc.next().charAt(0);
	               if(again == 'Y' || again == 'y') { id[f] = null; pass[f] = null; balance[f] = -1;
