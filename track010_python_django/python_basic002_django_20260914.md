...
```bash
# 타언어의 MVC 패턴과 거의 동일 (mvc = mtv) v=view , t=template, c=controller
# JAVA
# Model    - 데이터관련   : Entity / Dao / Dto
# Template - 화면구성     : HTML, JSP, Thymeleat ,,,
# View     - 비지니스 로직 : Controller ,,,
```
...
mv tempPjt tempProject
```

### 3-4. 분석용 애플리케이션(App) 생성
```bash
# 장고프로젝트는 여러 개의 App (기능단위 모듈)로 구성
# analytics 앱 생성
...
## M(odel) + T(html, css: 화면) + V(Controller: 비지니스 로직, db와 view 연결)
...
def dashboard_view(request):
    ...
    # 4. 템플릿(html)으로 전달할 때 바인딩객체 (Spring - Model, ModelAndView)
    context = {
...
urlpatterns = [
    # http://127.0.0.1:8000/dashboard/  요청 시 views.py 의 dashboard_view 연결
...
```python
# 2) analysis_pjt/urls.py 수정 [프로젝트 설정]
from django.contrib import admin
...
urlpatterns = [
    ...
    # /dashboard/ 주소로 들어오는 얘들
    path{'dashboard/', include('analytics.urls')}ㄴ,
    # http://127.0.0.1:8000 접속 시, /dashboard/로 자동 이동
                ...
                datasets: [
                    {
                        ...
                        data: chartVisitors,    // Y축 ( 방문자 수 ) 1번
