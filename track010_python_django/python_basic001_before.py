...
# ==============================================================================
# 1)  ; 사용하지않고
# 2)  {} 사용하지않고 들여쓰기로 코드블록 구분
# 3)  타입선언 명시하지않음 (int, String)

print("\n\n--- [1] Python 핵심 ---")
print("\n--- 0. 변수 선언 및 출력 ---")
### int, String / const, let, var 키워드가 필요하지않음
...
print(f"index의 -1번쨰 과일: {fruits[-1]}") # -1 맨뒤의 요소추출
...
# --- [작성 공간] ---
# for+if 60점 이상점수만   처리: 10점씩 가산  결과: 리스트
...
# np.arange(start, end) : start부터 end 전까지 연속된 배열

print("\n--- 1. 1차원 및 2차원 배열(NDArray) 생성 ---")
# 1. 배열
# shape  1차원배열 (5, )  2차원배열(2,3)  3차원배열 (2,2,3)
arr1 = np.array([10,20,30,40,50])
print(f"1차원 배열 : {arr1}")
print(f"Array Shape : {arr1.shape}")  # (5, ) 1차원 데이터가 5개
...
# 매출액이 30000 넘었나 [False, True, True]
sales_30000 = data_2d[:, 1] >= 30000
print(f"매출액 30000이상 필터링: {sales_30000}")
print(f"필터링된 실제 데이터: {data_2d[sales_30000]}")
...
# 표준편차 : 100,100,100 (평균 100) 표준편차0
#           0,100,200 (평균 100) 표준편차 큼
...
print(f"Axis=0 세로방향(열방향) 합계: {col_sums}")
# 방문자수, 매출액
# data_2d = np.array([
#     [150, 10000],
#     [300, 50000],
...
# total_pay = np.sum(pay)
# avg_orders = np.mean(order)
...
# head() : 상단 미리보기 (기본값 5)
# info() : 전체행갯수, 컬럼데이터 타입, Null(결측지) 존재여부 확인
...
df['avg_spend'] = df['sales_amount'] / df['visitor_count'] 

print(f"방문자수 200명 이상 필터링: {over_200_df}")
...
# tolist(): 파이썬 List `[]` 포맷변환  Chsrt.js , 그래프그릴때
